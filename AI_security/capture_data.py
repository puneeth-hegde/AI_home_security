...
# Folder for your face data
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_FOLDER = os.path.join(BASE_DIR, "dataset", "puneeth")

os.makedirs(SAVE_FOLDER, exist_ok=True)

# We use Haar Cascade for fast, local face finding (just to help us crop)
...
while True:
    ...
    # 2. Optimization: Detect on a small copy (Fast)
    ...
    # 4. Prepare Display (Fit to screen)
    ...
    # Draw box if face found
    if len(faces) > 0:
        # Take the largest face
        (x, y, w_box, h_box) = sorted(faces, key=lambda f: f[2] * f[3], reverse=True)[0]
        
        # --- Logic for SAVING (High Res Crop) ---
        # Add padding (hair/chin)
        pad = int(h_box * 0.2) 
    ...
    cv2.imshow("Smart Collector - Press S", display_frame)
    ...
    if key == ord('s') or key == ord('S'):
        if current_face_crop is not None and current_face_crop.size > 0:
            ...
            print("[ERROR] No face detected! Move closer until Green Box appears.")
